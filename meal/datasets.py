...
def get_cls_model(fold=0):
    ...
    checkpoint = torch.load(
        str(
            Path("/home/thuy0050/ft49_scratch/thuy0050/meal/")
            / "checkpoints"
            / f"cls_model_{fold}.pth"
        ),
...
def get_backbone_model(fold=0):
    ...
    model = DetectMultiBackend(
        ...
        weights=str(
            Path("/home/thuy0050/ft49_scratch/thuy0050/meal/runs/train")
            / f"train_backbone_fold_{fold}"
            / "weights"
            / f"best.pt"
        ),
...
def get_dataloaders(train_transforms, val_transforms, batch_size, fold=0):
    train_ds = VofoDataset(
        "./vocal-folds", train_val="train", transform=train_transforms, fold=fold
    )
    train_dl = DataLoader(
        train_ds,
    ...
    val_ds = VofoDataset(
        "./vocal-folds", train_val="val", fold=fold, transform=val_transforms
    )
    val_dl = DataLoader(
        val_ds,
        batch_size=batch_size,
    ...
    train_transforms = A.Compose(
        [
            ...
            A.augmentations.geometric.transforms.Affine(
                scale=0.5, translate_percent=0.1, p=0.3
            ),
            A.augmentations.transforms.ColorJitter(
                brightness=0.4, contrast=0.2, saturation=0.7, hue=0.015, p=0.3
            ),
        ],
...
class VofoDataset(Dataset):
    def __init__(
        self, root_dir, train_val, fold=0, transform=None, target_transform=None
    ):
        super().__init__()
        # self.root_dir = Path("./vocal-folds/")
        self.root_dir = Path("/home/thuy0050/ft49_scratch/thuy0050/meal/vocal-folds/")
        ...
        self.backbone_model = get_backbone_model(fold=fold)
        ...
        with open(
            self.root_dir / "annotations" / f"annotation_{fold}_{train_val}.json", "r"
        ) as f:
            js = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        with torch.no_grad():
            small, medium, large, _ = self.backbone_model(x, feature_map=True)
